...
def scored(q, mods=None):
    ...
    for m in (mods or list(by)):
        ...
        for s, r in zip(sc[0], rid[0]):
            it = items[int(r)]; out.append(((float(s) - it.get("bm", 0.0)) / it.get("bs", 1.0) + float(s), int(r)))
    out.sort(reverse=True)
    return [r for _, r in out]

def rank_of(order, tgt):
    for pos, r in enumerate(order, 1):
        if r in tgt: return pos
    return 0

per = {}; t1 = t3 = 0; rr = 0.0; n = 0; missing = []
print(f"{'modality':7} {'target':16} modal# mixed#")
for modality, key, q in M:
    tgt = set(targets(key))
    if not tgt: missing.append(f"{modality}:{key}"); continue
    n += 1
    mr = rank_of(scored(q, MOD_MAP[modality]), tgt)
    xr = rank_of(scored(q), tgt)
...
print(f"\nmixed corpus ({n} queries, {len(items)} items): "
      f"top-1 {t1}/{n} ({100*t1/n:.0f}%)  top-3 {t3}/{n} ({100*t3/n:.0f}%)  MRR {rr/n:.3f}")
